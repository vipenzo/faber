...
cur.rest)for(height=0;height<cur.rest.length;height++)updateWidgetHeight(cur.rest[height])}}}function updateWidgetHeight(line){if(line.widgets)for(var i=0;i<line.widgets.length;++i)line.widgets[i].height=line.widgets[i].node.parentNode.offsetHeight}function visibleLines(display,doc,viewport){var top=viewport&&null!=viewport.top?Math.max(0,viewport.top):display.scroller.scrollTop;top=Math.floor(top-display.lineSpace.offsetTop);var bottom=viewport&&null!=viewport.bottom?viewport.bottom:top+display.wrapper.clientHeight;
top=lineAtHeight(doc,top);bottom=lineAtHeight(doc,bottom);if(viewport&&viewport.ensure){var ensureFrom=viewport.ensure.from.line;viewport=viewport.ensure.to.line;ensureFrom<top?(top=ensureFrom,bottom=lineAtHeight(doc,heightAtLine(getLine(doc,ensureFrom))+display.wrapper.clientHeight)):Math.min(viewport,doc.lastLine())>=bottom&&(top=lineAtHeight(doc,heightAtLine(getLine(doc,viewport))-display.wrapper.clientHeight),bottom=viewport)}return{from:top,to:Math.max(bottom,top+1)}}function alignHorizontally(cm){var display=
cm.display,view=display.view;if(display.alignWidgets||display.gutters.firstChild&&cm.options.fixedGutter){for(var comp=compensateForHScroll(display)-display.scroller.scrollLeft+cm.doc.scrollLeft,gutterW=display.gutters.offsetWidth,left=comp+"px",i=0;i<view.length;i++)if(!view[i].hidden){cm.options.fixedGutter&&(view[i].gutter&&(view[i].gutter.style.left=left),view[i].gutterBackground&&(view[i].gutterBackground.style.left=left));var align=view[i].alignable;if(align)for(var j=0;j<align.length;j++)align[j].style.left=
left}cm.options.fixedGutter&&(display.gutters.style.left=comp+gutterW+"px")}}function maybeUpdateLineNumberWidth(cm){if(!cm.options.lineNumbers)return!1;var doc=cm.doc;doc=lineNumberFor(cm.options,doc.first+doc.size-1);var display=cm.display;if(doc.length!=display.lineNumChars){var test=display.measure.appendChild(elt$jscomp$0("div",[elt$jscomp$0("div",doc)],"CodeMirror-linenumber CodeMirror-gutter-elt")),innerW=test.firstChild.offsetWidth;test=test.offsetWidth-innerW;display.lineGutter.style.width=
"";display.lineNumInnerWidth=Math.max(innerW,display.lineGutter.offsetWidth-test)+1;display.lineNumWidth=display.lineNumInnerWidth+test;display.lineNumChars=display.lineNumInnerWidth?doc.length:-1;display.lineGutter.style.width=display.lineNumWidth+"px";updateGutterSpace(cm);return!0}return!1}function calculateScrollPos(cm,rect){var display=cm.display,snapMargin=textHeight(cm.display);0>rect.top&&(rect.top=0);var screentop=cm.curOp&&null!=cm.curOp.scrollTop?cm.curOp.scrollTop:display.scroller.scrollTop,
screen=displayHeight(cm),result={};rect.bottom-rect.top>screen&&(rect.bottom=rect.top+screen);var docBottom=cm.doc.height+paddingVert(display),atTop=rect.top<snapMargin;snapMargin=rect.bottom>docBottom-snapMargin;rect.top<screentop?result.scrollTop=atTop?0:rect.top:rect.bottom>screentop+screen&&(screen=Math.min(rect.top,(snapMargin?docBottom:rect.bottom)-screen),screen!=screentop&&(result.scrollTop=screen));screentop=cm.curOp&&null!=cm.curOp.scrollLeft?cm.curOp.scrollLeft:display.scroller.scrollLeft;
cm=displayWidth(cm)-(cm.options.fixedGutter?display.gutters.offsetWidth:0);if(display=rect.right-rect.left>cm)rect.right=rect.left+cm;10>rect.left?result.scrollLeft=0:rect.left<screentop?result.scrollLeft=Math.max(0,rect.left-(display?0:10)):rect.right>cm+screentop-3&&(result.scrollLeft=rect.right+(display?0:10)-cm);return result}function addToScrollTop(cm,top){null!=top&&(resolveScrollToPos(cm),cm.curOp.scrollTop=(null==cm.curOp.scrollTop?cm.doc.scrollTop:cm.curOp.scrollTop)+top)}function ensureCursorVisible(cm){resolveScrollToPos(cm);
...
cursorActivityCalled:0,selectionChanged:!1,updateMaxLine:!1,scrollLeft:null,scrollTop:null,scrollToPos:null,focus:!1,id:++nextOpId};cm=cm.curOp;operationGroup?operationGroup.ops.push(cm):cm.ownsGroup=operationGroup={ops:[cm],delayedCallbacks:[]}}function endOperation(cm$jscomp$0){finishOperation(cm$jscomp$0.curOp,function(group){for(var i=0;i<group.ops.length;i++)group.ops[i].cm.curOp=null;group=group.ops;for(i=0;i<group.length;i++){var op=group[i],cm=op.cm,display=cm.display,display$jscomp$0=cm.display;
!display$jscomp$0.scrollbarsClipped&&display$jscomp$0.scroller.offsetWidth&&(display$jscomp$0.nativeBarWidth=display$jscomp$0.scroller.offsetWidth-display$jscomp$0.scroller.clientWidth,display$jscomp$0.heightForcer.style.height=scrollGap(cm)+"px",display$jscomp$0.sizer.style.marginBottom=-display$jscomp$0.nativeBarWidth+"px",display$jscomp$0.sizer.style.borderRightWidth=scrollGap(cm)+"px",display$jscomp$0.scrollbarsClipped=!0);op.updateMaxLine&&findMaxLine(cm);op.mustUpdate=op.viewChanged||op.forceUpdate||
null!=op.scrollTop||op.scrollToPos&&(op.scrollToPos.from.line<display.viewFrom||op.scrollToPos.to.line>=display.viewTo)||display.maxLineChanged&&cm.options.lineWrapping;op.update=op.mustUpdate&&new DisplayUpdate(cm,op.mustUpdate&&{top:op.scrollTop,ensure:op.scrollToPos},op.forceUpdate)}for(i=0;i<group.length;i++)op=group[i],op.updatedDisplay=op.mustUpdate&&updateDisplayIfNeeded(op.cm,op.update);for(i=0;i<group.length;i++)if(op=group[i],cm=op.cm,display=cm.display,op.updatedDisplay&&updateHeightsInViewport(cm),
op.barMeasure=measureForScrollbars(cm),display.maxLineChanged&&!cm.options.lineWrapping&&(display$jscomp$0=display.maxLine.text.length,display$jscomp$0=measureCharPrepared(cm,prepareMeasureForLine(cm,display.maxLine),display$jscomp$0,void 0),op.adjustWidthTo=display$jscomp$0.left+3,cm.display.sizerWidth=op.adjustWidthTo,op.barMeasure.scrollWidth=Math.max(display.scroller.clientWidth,display.sizer.offsetLeft+op.adjustWidthTo+scrollGap(cm)+cm.display.barWidth),op.maxScrollLeft=Math.max(0,display.sizer.offsetLeft+
op.adjustWidthTo-displayWidth(cm))),op.updatedDisplay||op.selectionChanged)op.preparedSelection=display.input.prepareSelection();for(i=0;i<group.length;i++)op=group[i],cm=op.cm,null!=op.adjustWidthTo&&(cm.display.sizer.style.minWidth=op.adjustWidthTo+"px",op.maxScrollLeft<cm.doc.scrollLeft&&setScrollLeft(cm,Math.min(cm.display.scroller.scrollLeft,op.maxScrollLeft),!0),cm.display.maxLineChanged=!1),display=op.focus&&op.focus==activeElt(),op.preparedSelection&&cm.display.input.showSelection(op.preparedSelection,
...
(-1<indexOf(lineView.changes,"gutter")&&(cur=!1),updateLineForChanges(cm,lineView,display,dims));cur&&(removeChildren(lineView.lineNumber),lineView.lineNumber.appendChild(document.createTextNode(lineNumberFor(cm.options,display))));cur=lineView.node.nextSibling}else{var node$jscomp$0=buildLineElement(cm,lineView,display,dims);container.insertBefore(node$jscomp$0,cur)}display+=lineView.size}for(;cur;)cur=rm(cur)}function updateGutterSpace(cm){cm.display.sizer.style.marginLeft=cm.display.gutters.offsetWidth+
"px"}function setDocumentHeight(cm,measure){cm.display.sizer.style.minHeight=measure.docHeight+"px";cm.display.heightForcer.style.top=measure.docHeight+"px";cm.display.gutters.style.height=measure.docHeight+cm.display.barHeight+scrollGap(cm)+"px"}function updateGutters(cm){var gutters=cm.display.gutters,specs=cm.options.gutters;removeChildren(gutters);for(var i=0;i<specs.length;++i){var gutterClass=specs[i],gElt=gutters.appendChild(elt$jscomp$0("div",null,"CodeMirror-gutter "+gutterClass));"CodeMirror-linenumbers"==
gutterClass&&(cm.display.lineGutter=gElt,gElt.style.width=(cm.display.lineNumWidth||1)+"px")}gutters.style.display=i?"":"none";updateGutterSpace(cm)}function setGuttersForLineNumbers(options){var found=indexOf(options.gutters,"CodeMirror-linenumbers");-1==found&&options.lineNumbers?options.gutters=options.gutters.concat(["CodeMirror-linenumbers"]):-1<found&&!options.lineNumbers&&(options.gutters=options.gutters.slice(0),options.gutters.splice(found,1))}function wheelEventDelta(e){var dx=e.wheelDeltaX,
dy=e.wheelDeltaY;null==dx&&e.detail&&e.axis==e.HORIZONTAL_AXIS&&(dx=e.detail);null==dy&&e.detail&&e.axis==e.VERTICAL_AXIS?dy=e.detail:null==dy&&(dy=e.wheelDelta);return{x:dx,y:dy}}function wheelEventPixels(e){e=wheelEventDelta(e);e.x*=wheelPixelsPerUnit;e.y*=wheelPixelsPerUnit;return e}function onScrollWheel(cm,e){var delta=wheelEventDelta(e),dx=delta.x;delta=delta.y;var display=cm.display,scroll=display.scroller,canScrollX=scroll.scrollWidth>scroll.clientWidth,canScrollY=scroll.scrollHeight>scroll.clientHeight;
if(dx&&canScrollX||delta&&canScrollY){if(delta&&mac&&webkit){canScrollX=e.target;var view=display.view;a:for(;canScrollX!=scroll;canScrollX=canScrollX.parentNode)for(var i=0;i<view.length;i++)if(view[i].node==canScrollX){cm.display.currentWheelTarget=canScrollX;break a}}!dx||gecko||presto||null==wheelPixelsPerUnit?(delta&&null!=wheelPixelsPerUnit&&(e=delta*wheelPixelsPerUnit,canScrollY=cm.doc.scrollTop,canScrollX=canScrollY+display.wrapper.clientHeight,0>e?canScrollY=Math.max(0,canScrollY+e-50):canScrollX=
...
display.wheelStartX=display.wheelStartY=null;movedX&&(wheelPixelsPerUnit=(wheelPixelsPerUnit*wheelSamples+movedX)/(wheelSamples+1),++wheelSamples)}},200)):(display.wheelDX+=dx,display.wheelDY+=delta))):(delta&&canScrollY&&updateScrollTop(cm,Math.max(0,scroll.scrollTop+delta*wheelPixelsPerUnit)),setScrollLeft(cm,Math.max(0,scroll.scrollLeft+dx*wheelPixelsPerUnit)),(!delta||delta&&canScrollY)&&e_preventDefault(e),display.wheelStartX=null)}}function normalizeSelection(ranges,primIndex){primIndex=ranges[primIndex];
ranges.sort(function(a,b){return cmp(a.from(),b.from())});primIndex=indexOf(ranges,primIndex);for(var i=1;i<ranges.length;i++){var cur=ranges[i],prev=ranges[i-1];if(0<=cmp(prev.to(),cur.from())){var from=minPos(prev.from(),cur.from()),to=maxPos(prev.to(),cur.to());cur=prev.empty()?cur.from()==cur.head:prev.from()==prev.head;i<=primIndex&&--primIndex;ranges.splice(--i,2,new Range(cur?to:from,cur?from:to))}}return new Selection(ranges,primIndex)}function simpleSelection(anchor,head){return new Selection([new Range(anchor,
...
change);addChangeToHistory(doc$jscomp$0,change,selAfter,doc$jscomp$0.cm?doc$jscomp$0.cm.curOp.id:NaN);makeChangeSingleDoc(doc$jscomp$0,change,selAfter,stretchSpansOverChange(doc$jscomp$0,change));var rebased=[];linkedDocs(doc$jscomp$0,function(doc,sharedHist){sharedHist||-1!=indexOf(rebased,doc.history)||(rebaseHist(doc.history,change),rebased.push(doc.history));makeChangeSingleDoc(doc,change,null,stretchSpansOverChange(doc,change))})}}function makeChangeFromHistory(doc$jscomp$0,type,allowSelectionOnly){if(!doc$jscomp$0.cm||
!doc$jscomp$0.cm.state.suppressEdits||allowSelectionOnly){for(var hist=doc$jscomp$0.history,event,selAfter=doc$jscomp$0.sel,source="undo"==type?hist.done:hist.undone,dest="undo"==type?hist.undone:hist.done,i$jscomp$0=0;i$jscomp$0<source.length&&(event=source[i$jscomp$0],allowSelectionOnly?!event.ranges||event.equals(doc$jscomp$0.sel):event.ranges);i$jscomp$0++);if(i$jscomp$0!=source.length){for(hist.lastOrigin=hist.lastSelOrigin=null;;)if(event=source.pop(),event.ranges){pushSelectionToHistory(event,
dest);if(allowSelectionOnly&&!event.equals(doc$jscomp$0.sel)){setSelection(doc$jscomp$0,event,{clearRedo:!1});return}selAfter=event}else break;var antiChanges=[];pushSelectionToHistory(selAfter,dest);dest.push({changes:antiChanges,generation:hist.generation});hist.generation=event.generation||++hist.maxGeneration;var filter=hasHandler(doc$jscomp$0,"beforeChange")||doc$jscomp$0.cm&&hasHandler(doc$jscomp$0.cm,"beforeChange");allowSelectionOnly=function(i){var change=event.changes[i];change.origin=type;
if(filter&&!filterChange(doc$jscomp$0,change,!1))return source.length=0,{};antiChanges.push(historyChangeFromChange(doc$jscomp$0,change));var after=i?computeSelAfterChange(doc$jscomp$0,change):lst(source);makeChangeSingleDoc(doc$jscomp$0,change,after,mergeOldSpans(doc$jscomp$0,change));!i&&doc$jscomp$0.cm&&doc$jscomp$0.cm.scrollIntoView({from:change.from,to:changeEnd(change)});var rebased=[];linkedDocs(doc$jscomp$0,function(doc,sharedHist){sharedHist||-1!=indexOf(rebased,doc.history)||(rebaseHist(doc.history,
change),rebased.push(doc.history));makeChangeSingleDoc(doc,change,null,mergeOldSpans(doc,change))})};for(hist=event.changes.length-1;0<=hist;--hist)if(selAfter=allowSelectionOnly(hist))return selAfter.v}}}function shiftDoc(doc,distance){if(0!=distance&&(doc.first+=distance,doc.sel=new Selection(map(doc.sel.ranges,function(range){return new Range(Pos(range.anchor.line+distance,range.anchor.ch),Pos(range.head.line+distance,range.head.ch))}),doc.sel.primIndex),doc.cm)){regChange(doc.cm,doc.first,doc.first-
distance,distance);for(var d=doc.cm.display,l=d.viewFrom;l<d.viewTo;l++)regLineChange(doc.cm,l,"gutter")}}function makeChangeSingleDoc(doc,change,selAfter,spans){if(doc.cm&&!doc.cm.curOp)return operation(doc.cm,makeChangeSingleDoc)(doc,change,selAfter,spans);if(change.to.line<doc.first)shiftDoc(doc,change.text.length-1-(change.to.line-change.from.line));else if(!(change.from.line>doc.lastLine())){if(change.from.line<doc.first){var shift=change.text.length-1-(doc.first-change.from.line);shiftDoc(doc,
shift);change={from:Pos(doc.first,0),to:Pos(change.to.line+shift,change.to.ch),text:[lst(change.text)],origin:change.origin}}shift=doc.lastLine();change.to.line>shift&&(change={from:change.from,to:Pos(shift,getLine(doc,shift).text.length),text:[change.text[0]],origin:change.origin});change.removed=getBetween(doc,change.from,change.to);selAfter||(selAfter=computeSelAfterChange(doc,change));doc.cm?makeChangeSingleDocInEditor(doc.cm,change,spans):updateDoc(doc,change,spans);setSelectionNoUndo(doc,selAfter,
...
(doc.iter(checkWidthStart,from.line+change.text.length,function(line){var len=lineLength(line);len>display.maxLineLength&&(display.maxLine=line,display.maxLineLength=len,display.maxLineChanged=!0,recomputeMaxLength=!1)}),recomputeMaxLength&&(cm.curOp.updateMaxLine=!0));retreatFrontier(doc,from.line);startWorker(cm,400);spans=change.text.length-(to.line-from.line)-1;change.full?regChange(cm):from.line!=to.line||1!=change.text.length||isWholeLineUpdate(cm.doc,change)?regChange(cm,from.line,to.line+
1,spans):regLineChange(cm,from.line,"text");spans=hasHandler(cm,"changes");if((doc=hasHandler(cm,"change"))||spans)change={from:from,to:to,text:change.text,removed:change.removed,origin:change.origin},doc&&signalLater(cm,"change",cm,change),spans&&(cm.curOp.changeObjs||(cm.curOp.changeObjs=[])).push(change);cm.display.selForContextMenu=null}function replaceRange(doc,code,from,to,origin){to||(to=from);if(0>cmp(to,from)){var assign=[to,from];from=assign[0];to=assign[1];assign}"string"==typeof code&&
(code=doc.splitLines(code));makeChange(doc,{from:from,to:to,text:code,origin:origin})}function rebaseHistSelSingle(pos,from,to,diff){to<pos.line?pos.line+=diff:from<pos.line&&(pos.line=from,pos.ch=0)}function rebaseHistArray(array,from,to,diff){for(var i=0;i<array.length;++i){var sub=array[i],ok=!0;if(sub.ranges)for(sub.copied||(sub=array[i]=sub.deepCopy(),sub.copied=!0),ok=0;ok<sub.ranges.length;ok++)rebaseHistSelSingle(sub.ranges[ok].anchor,from,to,diff),rebaseHistSelSingle(sub.ranges[ok].head,
...
0);var span=new MarkedSpan(marker,curLine==from.line?from.ch:null,curLine==to.line?to.ch:null);line.markedSpans=line.markedSpans?line.markedSpans.concat([span]):[span];span.marker.attachLine(line);++curLine});marker.collapsed&&doc.iter(from.line,to.line+1,function(line){lineIsHidden(doc,line)&&updateLineHeight(line,0)});marker.clearOnEnter&&on(marker,"beforeCursorEnter",function(){return marker.clear()});marker.readOnly&&(sawReadOnlySpans=!0,(doc.history.done.length||doc.history.undone.length)&&doc.clearHistory());
marker.collapsed&&(marker.id=++nextMarkerId,marker.atomic=!0);if(cm){updateMaxLine&&(cm.curOp.updateMaxLine=!0);if(marker.collapsed)regChange(cm,from.line,to.line+1);else if(marker.className||marker.title||marker.startStyle||marker.endStyle||marker.css)for(options=from.line;options<=to.line;options++)regLineChange(cm,options,"text");marker.atomic&&reCheckSelection(cm.doc);signalLater(cm,"markerAdded",cm,marker)}return marker}function markTextShared(doc$jscomp$0,from,to,options,type){options=copyObj(options);
options.shared=!1;var markers=[markText(doc$jscomp$0,from,to,options,type)],primary=markers[0],widget=options.widgetNode;linkedDocs(doc$jscomp$0,function(doc){widget&&(options.widgetNode=widget.cloneNode(!0));markers.push(markText(doc,clipPos(doc,from),clipPos(doc,to),options,type));for(var i=0;i<doc.linked.length;++i)if(doc.linked[i].isParent)return;primary=lst(markers)});return new SharedTextMarker(markers,primary)}function findSharedMarkers(doc){return doc.findMarks(Pos(doc.first,0),doc.clipPos(Pos(doc.lastLine())),
function(m){return m.parent})}function detachSharedMarkers(markers){for(var loop=function(i){i=markers[i];var linked=[i.primary.doc];linkedDocs(i.primary.doc,function(d){return linked.push(d)});for(var j=0;j<i.markers.length;j++){var subMarker=i.markers[j];-1==indexOf(linked,subMarker.doc)&&(subMarker.parent=null,i.markers.splice(j--,1))}},i$jscomp$0=0;i$jscomp$0<markers.length;i$jscomp$0++)loop(i$jscomp$0)}function onDrop(e){var cm=this;clearDragCursor(cm);if(!signalDOMEvent(cm,e)&&!eventInWidget(cm.display,
e)){e_preventDefault(e);ie&&(lastDrop=+new Date);var pos=posFromMouse(cm,e,!0),files=e.dataTransfer.files;if(pos&&!cm.isReadOnly())if(files&&files.length&&window.FileReader&&window.File)for(var n=files.length,text=Array(n),read=0,loadFile=function(file,i){if(!cm.options.allowDropFileTypes||-1!=indexOf(cm.options.allowDropFileTypes,file.type)){var reader=new FileReader;reader.onload=operation(cm,function(){var content=reader.result;/[\x00-\x08\x0e-\x1f]{2}/.test(content)&&(content="");text[i]=content;
++read==n&&(pos=clipPos(cm.doc,pos),content={from:pos,to:pos,text:cm.doc.splitLines(text.join(cm.doc.lineSeparator())),origin:"paste"},makeChange(cm.doc,content),setSelectionReplaceHistory(cm.doc,simpleSelection(pos,changeEnd(content))))});reader.readAsText(file)}},i$jscomp$0=0;i$jscomp$0<n;++i$jscomp$0)loadFile(files[i$jscomp$0],i$jscomp$0);else if(cm.state.draggingText&&-1<cm.doc.sel.contains(pos))cm.state.draggingText(e),setTimeout(function(){return cm.display.input.focus()},20);else try{if(loadFile=
e.dataTransfer.getData("Text")){cm.state.draggingText&&!cm.state.draggingText.copy&&(i$jscomp$0=cm.listSelections());setSelectionNoUndo(cm.doc,simpleSelection(pos,pos));if(i$jscomp$0)for(files=0;files<i$jscomp$0.length;++files)replaceRange(cm.doc,"",i$jscomp$0[files].anchor,i$jscomp$0[files].head,"drag");cm.replaceSelection(loadFile,"around","paste");cm.display.input.focus()}}catch(e$1){}}}function clearDragCursor(cm){cm.display.dragCursor&&(cm.display.lineSpace.removeChild(cm.display.dragCursor),
cm.display.dragCursor=null)}function forEachCodeMirror(f){if(document.getElementsByClassName)for(var byClass=document.getElementsByClassName("CodeMirror"),i=0;i<byClass.length;i++){var cm=byClass[i].CodeMirror;cm&&f(cm)}}function registerGlobalHandlers(){var resizeTimer;on(window,"resize",function(){null==resizeTimer&&(resizeTimer=setTimeout(function(){resizeTimer=null;forEachCodeMirror(onResize)},100))});on(window,"blur",function(){return forEachCodeMirror(onBlur)})}function onResize(cm){var d=cm.display;
if(d.lastWrapHeight!=d.wrapper.clientHeight||d.lastWrapWidth!=d.wrapper.clientWidth)d.cachedCharWidth=d.cachedTextHeight=d.cachedPaddingH=null,d.scrollbarsClipped=!1,cm.setSize()}function normalizeKeyName(name){var parts=name.split(/-(?!$)/);name=parts[parts.length-1];for(var alt,ctrl,shift,cmd,i=0;i<parts.length-1;i++){var mod=parts[i];if(/^(cmd|meta|m)$/i.test(mod))cmd=!0;else if(/^a(lt)?$/i.test(mod))alt=!0;else if(/^(c|ctrl|control)$/i.test(mod))ctrl=!0;else if(/^s(hift)?$/i.test(mod))shift=!0;
else throw Error("Unrecognized modifier name: "+mod);}alt&&(name="Alt-"+name);ctrl&&(name="Ctrl-"+name);cmd&&(name="Cmd-"+name);shift&&(name="Shift-"+name);return name}function normalizeKeyMap(keymap){var copy={},keyname;for(keyname in keymap)if(keymap.hasOwnProperty(keyname)){var value=keymap[keyname];if(!/^(name|fallthrough|(de|at)tach)$/.test(keyname)){if("..."!=value)for(var keys=map(keyname.split(" "),normalizeKeyName),i=0;i<keys.length;i++){if(i==keys.length-1){var name=keys.join(" ");var val=
value}else name=keys.slice(0,i+1).join(" "),val="...";var prev=copy[name];if(!prev)copy[name]=val;else if(prev!=val)throw Error("Inconsistent bindings for "+name);}delete keymap[keyname]}}for(var prop in copy)keymap[prop]=copy[prop];return keymap}function lookupKey(key,map$$1,handle,context){map$$1=getKeyMap(map$$1);var found=map$$1.call?map$$1.call(key,context):map$$1[key];if(!1===found)return"nothing";if("..."===found)return"multi";if(null!=found&&handle(found))return"handled";if(map$$1.fallthrough){if("[object Array]"!=
Object.prototype.toString.call(map$$1.fallthrough))return lookupKey(key,map$$1.fallthrough,handle,context);for(found=0;found<map$$1.fallthrough.length;found++){var result=lookupKey(key,map$$1.fallthrough[found],handle,context);if(result)return result}}}function isModifierKey(value){value="string"==typeof value?value:keyNames[value.keyCode];return"Ctrl"==value||"Alt"==value||"Shift"==value||"Mod"==value}function addModifierNames(name,event,noShift){var base=name;event.altKey&&"Alt"!=base&&(name="Alt-"+
name);(flipCtrlCmd?event.metaKey:event.ctrlKey)&&"Ctrl"!=base&&(name="Ctrl-"+name);(flipCtrlCmd?event.ctrlKey:event.metaKey)&&"Cmd"!=base&&(name="Cmd-"+name);!noShift&&event.shiftKey&&"Shift"!=base&&(name="Shift-"+name);return name}function keyName(event,noShift){if(presto&&34==event.keyCode&&event["char"])return!1;var name=keyNames[event.keyCode];return null==name||event.altGraphKey?!1:addModifierNames(name,event,noShift)}function getKeyMap(val){return"string"==typeof val?keyMap[val]:val}function deleteNearSelection(cm,
...
ch+dir,dir);return 0>ch||ch>line.text.length?null:ch}function moveLogically(line,start,dir){line=moveCharLogically(line,start.ch,dir);return null==line?null:new Pos(start.line,line,0>dir?"after":"before")}function endOfLine(visually,cm,lineObj,lineNo,dir){if(visually&&(visually=getOrder(lineObj,cm.doc.direction))){visually=0>dir?lst(visually):visually[0];var sticky=0>dir==(1==visually.level)?"after":"before";if(0<visually.level||"rtl"==cm.doc.direction){var prep=prepareMeasureForLine(cm,lineObj);
var ch$jscomp$0=0>dir?lineObj.text.length-1:0;var targetTop=measureCharPrepared(cm,prep,ch$jscomp$0).top;ch$jscomp$0=findFirst(function(ch){return measureCharPrepared(cm,prep,ch).top==targetTop},0>dir==(1==visually.level)?visually.from:visually.to-1,ch$jscomp$0);"before"==sticky&&(ch$jscomp$0=moveCharLogically(lineObj,ch$jscomp$0,1))}else ch$jscomp$0=0>dir?visually.to:visually.from;return new Pos(lineNo,ch$jscomp$0,sticky)}return new Pos(lineNo,0>dir?lineObj.text.length:0,0>dir?"before":"after")}
function moveVisually(cm,line,start,dir$jscomp$0){var bidi=getOrder(line,cm.doc.direction);if(!bidi)return moveLogically(line,start,dir$jscomp$0);start.ch>=line.text.length?(start.ch=line.text.length,start.sticky="before"):0>=start.ch&&(start.ch=0,start.sticky="after");var partPos$jscomp$0=getBidiPartAt(bidi,start.ch,start.sticky),part$jscomp$0=bidi[partPos$jscomp$0];if("ltr"==cm.doc.direction&&0==part$jscomp$0.level%2&&(0<dir$jscomp$0?part$jscomp$0.to>start.ch:part$jscomp$0.from<start.ch))return moveLogically(line,
start,dir$jscomp$0);var mv=function(pos,dir){return moveCharLogically(line,pos instanceof Pos?pos.ch:pos,dir)},prep,getWrappedLineExtent=function(ch){if(!cm.options.lineWrapping)return{begin:0,end:line.text.length};prep=prep||prepareMeasureForLine(cm,line);return wrappedLineExtentChar(cm,line,prep,ch)},wrappedLineExtent$jscomp$0=getWrappedLineExtent("before"==start.sticky?mv(start,-1):start.ch);if("rtl"==cm.doc.direction||1==part$jscomp$0.level){var moveInStorageOrder$jscomp$2=1==part$jscomp$0.level==
0>dir$jscomp$0,ch$jscomp$1=mv(start,moveInStorageOrder$jscomp$2?1:-1);if(null!=ch$jscomp$1&&(moveInStorageOrder$jscomp$2?ch$jscomp$1<=part$jscomp$0.to&&ch$jscomp$1<=wrappedLineExtent$jscomp$0.end:ch$jscomp$1>=part$jscomp$0.from&&ch$jscomp$1>=wrappedLineExtent$jscomp$0.begin))return new Pos(start.line,ch$jscomp$1,moveInStorageOrder$jscomp$2?"before":"after")}part$jscomp$0=function(partPos,dir,wrappedLineExtent){for(var getRes=function(ch,moveInStorageOrder){return moveInStorageOrder?new Pos(start.line,
mv(ch,1),"before"):new Pos(start.line,ch,"after")};0<=partPos&&partPos<bidi.length;partPos+=dir){var part=bidi[partPos],moveInStorageOrder$jscomp$0=0<dir==(1!=part.level),ch$jscomp$0=moveInStorageOrder$jscomp$0?wrappedLineExtent.begin:mv(wrappedLineExtent.end,-1);if(part.from<=ch$jscomp$0&&ch$jscomp$0<part.to)return getRes(ch$jscomp$0,moveInStorageOrder$jscomp$0);ch$jscomp$0=moveInStorageOrder$jscomp$0?part.from:mv(part.to,-1);if(wrappedLineExtent.begin<=ch$jscomp$0&&ch$jscomp$0<wrappedLineExtent.end)return getRes(ch$jscomp$0,
moveInStorageOrder$jscomp$0)}};if(partPos$jscomp$0=part$jscomp$0(partPos$jscomp$0+dir$jscomp$0,dir$jscomp$0,wrappedLineExtent$jscomp$0))return partPos$jscomp$0;wrappedLineExtent$jscomp$0=0<dir$jscomp$0?wrappedLineExtent$jscomp$0.end:mv(wrappedLineExtent$jscomp$0.begin,-1);return null==wrappedLineExtent$jscomp$0||0<dir$jscomp$0&&wrappedLineExtent$jscomp$0==line.text.length||!(partPos$jscomp$0=part$jscomp$0(0<dir$jscomp$0?0:bidi.length-1,dir$jscomp$0,getWrappedLineExtent(wrappedLineExtent$jscomp$0)))?
null:partPos$jscomp$0}function lineStart(cm,lineN){var line=getLine(cm.doc,lineN),visual=visualLine(line);visual!=line&&(lineN=lineNo(visual));return endOfLine(!0,cm,visual,lineN,1)}function lineStartSmart(cm,pos){var start=lineStart(cm,pos.line),line=getLine(cm.doc,start.line);cm=getOrder(line,cm.doc.direction);return cm&&0!=cm[0].level?start:(line=Math.max(0,line.text.search(/\S/)),Pos(start.line,pos.line==start.line&&pos.ch<=line&&pos.ch?0:line,start.sticky))}function doHandleBinding(cm,bound,
dropShift){if("string"==typeof bound&&(bound=commands[bound],!bound))return!1;cm.display.input.ensurePolled();var prevShift=cm.display.shift,done=!1;try{cm.isReadOnly()&&(cm.state.suppressEdits=!0),dropShift&&(cm.display.shift=!1),done=bound(cm)!=Pass}finally{cm.display.shift=prevShift,cm.state.suppressEdits=!1}return done}function lookupKeyForEditor(cm,name,handle){for(var i=0;i<cm.state.keyMaps.length;i++){var result=lookupKey(name,cm.state.keyMaps[i],handle,cm);if(result)return result}return cm.options.extraKeys&&
lookupKey(name,cm.options.extraKeys,handle,cm)||lookupKey(name,cm.options.keyMap,handle,cm)}function dispatchKey(cm,name,e,handle){var seq=cm.state.keySeq;if(seq){if(isModifierKey(name))return"handled";stopSeq.set(50,function(){cm.state.keySeq==seq&&(cm.state.keySeq=null,cm.display.input.reset())});name=seq+" "+name}handle=lookupKeyForEditor(cm,name,handle);"multi"==handle&&(cm.state.keySeq=name);"handled"==handle&&signalLater(cm,"keyHandled",cm,name,e);if("handled"==handle||"multi"==handle)e_preventDefault(e),
restartBlink(cm);return seq&&!handle&&/'$/.test(name)?(e_preventDefault(e),!0):!!handle}function handleKeyBinding(cm,e){var name=keyName(e,!0);return name?e.shiftKey&&!cm.state.keySeq?dispatchKey(cm,"Shift-"+name,e,function(b){return doHandleBinding(cm,b,!0)})||dispatchKey(cm,name,e,function(b){if("string"==typeof b?/^go[A-Z]/.test(b):b.motion)return doHandleBinding(cm,b)}):dispatchKey(cm,name,e,function(b){return doHandleBinding(cm,b)}):!1}function handleCharBinding(cm,e,ch){return dispatchKey(cm,
"'"+ch+"'",e,function(b){return doHandleBinding(cm,b,!0)})}function onKeyDown(e){this.curOp.focus=activeElt();if(!signalDOMEvent(this,e)){ie&&11>ie_version&&27==e.keyCode&&(e.returnValue=!1);var code=e.keyCode;this.display.shift=16==code||e.shiftKey;var handled=handleKeyBinding(this,e);presto&&(lastStoppedKey=handled?code:null,!handled&&88==code&&!hasCopyEvent&&(mac?e.metaKey:e.ctrlKey)&&this.replaceSelection("",null,"cut"));18!=code||/\bCodeMirror-crosshair\b/.test(this.display.lineDiv.className)||
showCrossHair(this)}}function showCrossHair(cm){function up(e){18!=e.keyCode&&e.altKey||(rmClass(lineDiv,"CodeMirror-crosshair"),off(document,"keyup",up),off(document,"mouseover",up))}var lineDiv=cm.display.lineDiv;addClass(lineDiv,"CodeMirror-crosshair");on(document,"keyup",up);on(document,"mouseover",up)}function onKeyUp(e){16==e.keyCode&&(this.doc.sel.shift=!1);signalDOMEvent(this,e)}function onKeyPress(e){if(!(eventInWidget(this.display,e)||signalDOMEvent(this,e)||e.ctrlKey&&!e.altKey||mac&&e.metaKey)){var keyCode=
e.keyCode,charCode=e.charCode;if(presto&&keyCode==lastStoppedKey)lastStoppedKey=null,e_preventDefault(e);else if(!presto||e.which&&!(10>e.which)||!handleKeyBinding(this,e))if(keyCode=String.fromCharCode(null==charCode?keyCode:charCode),"\b"!=keyCode&&!handleCharBinding(this,e,keyCode))this.display.input.onKeyPress(e)}}function clickRepeat(pos,button){var now=+new Date;if(lastDoubleClick&&lastDoubleClick.compare(now,pos,button))return lastClick=lastDoubleClick=null,"triple";if(lastClick&&lastClick.compare(now,
pos,button))return lastDoubleClick=new PastClick(now,pos,button),lastClick=null,"double";lastClick=new PastClick(now,pos,button);lastDoubleClick=null;return"single"}function onMouseDown(e){var display=this.display;if(!(signalDOMEvent(this,e)||display.activeTouch&&display.input.supportsTouch()))if(display.input.ensurePolled(),display.shift=e.shiftKey,eventInWidget(display,e))webkit||(display.scroller.draggable=!1,setTimeout(function(){return display.scroller.draggable=!0},100));else if(!gutterEvent(this,
e,"gutterClick",!0)){var pos=posFromMouse(this,e),button=e_button(e),repeat=pos?clickRepeat(pos,button):"single";window.focus();1==button&&this.state.selectingText&&this.state.selectingText(e);pos&&handleMappedButton(this,button,pos,repeat,e)||(1==button?pos?leftButtonDown(this,pos,repeat,e):(e.target||e.srcElement)==display.scroller&&e_preventDefault(e):2==button?(pos&&extendSelection(this.doc,pos),setTimeout(function(){return display.input.focus()},20)):3==button&&(captureRightClick?onContextMenu(this,
e):delayBlurEvent(this)))}}function handleMappedButton(cm,button,pos,repeat,event){var name="Click";"double"==repeat?name="Double"+name:"triple"==repeat&&(name="Triple"+name);return dispatchKey(cm,addModifierNames((1==button?"Left":2==button?"Middle":"Right")+name,event),event,function(bound){"string"==typeof bound&&(bound=commands[bound]);if(!bound)return!1;var done=!1;try{cm.isReadOnly()&&(cm.state.suppressEdits=!0),done=bound(cm,pos)!=Pass}finally{cm.state.suppressEdits=!1}return done})}function leftButtonDown(cm,
pos,repeat,event){ie?setTimeout(bind(ensureFocus,cm),0):cm.curOp.focus=activeElt();var option=cm.getOption("configureMouse");option=option?option(cm,repeat,event):{};null==option.unit&&(option.unit=(chromeOS?event.shiftKey&&event.metaKey:event.altKey)?"rectangle":"single"==repeat?"char":"double"==repeat?"word":"line");if(null==option.extend||cm.doc.extend)option.extend=cm.doc.extend||event.shiftKey;null==option.addNew&&(option.addNew=mac?event.metaKey:event.ctrlKey);null==option.moveOnDrag&&(option.moveOnDrag=
...
!1;counter=Infinity;e_preventDefault(e);display.input.focus();off(document,"mousemove",move);off(document,"mouseup",up);doc.history.lastSelOrigin=null}var display=cm.display,doc=cm.doc;e_preventDefault(event);var startSel=doc.sel,ranges$jscomp$0=startSel.ranges;if(behavior.addNew&&!behavior.extend){var ourIndex=doc.sel.contains(start);var ourRange=-1<ourIndex?ranges$jscomp$0[ourIndex]:new Range(start,start)}else ourRange=doc.sel.primary(),ourIndex=doc.sel.primIndex;"rectangle"==behavior.unit?(behavior.addNew||
(ourRange=new Range(start,start)),start=posFromMouse(cm,event,!0,!0),ourIndex=-1):(event=rangeForUnit(cm,start,behavior.unit),ourRange=behavior.extend?extendRange(ourRange,event.anchor,event.head,behavior.extend):event);behavior.addNew?-1==ourIndex?(ourIndex=ranges$jscomp$0.length,setSelection(doc,normalizeSelection(ranges$jscomp$0.concat([ourRange]),ourIndex),{scroll:!1,origin:"*mouse"})):1<ranges$jscomp$0.length&&ranges$jscomp$0[ourIndex].empty()&&"char"==behavior.unit&&!behavior.extend?(setSelection(doc,
normalizeSelection(ranges$jscomp$0.slice(0,ourIndex).concat(ranges$jscomp$0.slice(ourIndex+1)),0),{scroll:!1,origin:"*mouse"}),startSel=doc.sel):replaceOneSelection(doc,ourIndex,ourRange,sel_mouse):(ourIndex=0,setSelection(doc,new Selection([ourRange],0),sel_mouse),startSel=doc.sel);var lastPos=start,editorSize=display.wrapper.getBoundingClientRect(),counter=0,move=operation(cm,function(e){e_button(e)?extend(e):done(e)}),up=operation(cm,done);cm.state.selectingText=up;on(document,"mousemove",move);
on(document,"mouseup",up)}function bidiSimplify(cm,range$$1){var anchor=range$$1.anchor,head=range$$1.head,anchorLine=getLine(cm.doc,anchor.line);if(0==cmp(anchor,head)&&anchor.sticky==head.sticky)return range$$1;anchorLine=getOrder(anchorLine);if(!anchorLine)return range$$1;var index=getBidiPartAt(anchorLine,anchor.ch,anchor.sticky),part=anchorLine[index];if(part.from!=anchor.ch&&part.to!=anchor.ch)return range$$1;var boundary=index+(part.from==anchor.ch==(1!=part.level)?0:1);if(0==boundary||boundary==
anchorLine.length)return range$$1;head.line!=anchor.line?cm=0<(head.line-anchor.line)*("ltr"==cm.doc.direction?1:-1):(cm=getBidiPartAt(anchorLine,head.ch,head.sticky),index=cm-index||(head.ch-anchor.ch)*(1==part.level?-1:1),cm=cm==boundary-1||cm==boundary?0>index:0<index);anchorLine=anchorLine[boundary+(cm?-1:0)];anchorLine=(boundary=cm==(1==anchorLine.level))?anchorLine.from:anchorLine.to;boundary=boundary?"after":"before";return anchor.ch==anchorLine&&anchor.sticky==boundary?range$$1:new Range(new Pos(anchor.line,
anchorLine,boundary),head)}function gutterEvent(cm,e,type,prevent){if(e.touches){var mX=e.touches[0].clientX;var mY=e.touches[0].clientY}else try{mX=e.clientX,mY=e.clientY}catch(e$2){return!1}if(mX>=Math.floor(cm.display.gutters.getBoundingClientRect().right))return!1;prevent&&e_preventDefault(e);prevent=cm.display;var lineBox=prevent.lineDiv.getBoundingClientRect();if(mY>lineBox.bottom||!hasHandler(cm,type))return e_defaultPrevented(e);mY-=lineBox.top-prevent.viewOffset;for(lineBox=0;lineBox<cm.options.gutters.length;++lineBox){var g=
prevent.gutters.childNodes[lineBox];if(g&&g.getBoundingClientRect().right>=mX)return mX=lineAtHeight(cm.doc,mY),signal(cm,type,cm,mX,cm.options.gutters[lineBox],e),e_defaultPrevented(e)}}function onContextMenu(cm,e){var JSCompiler_temp;(JSCompiler_temp=eventInWidget(cm.display,e))||(JSCompiler_temp=hasHandler(cm,"gutterContextMenu")?gutterEvent(cm,e,"gutterContextMenu",!1):!1);if(!JSCompiler_temp&&!signalDOMEvent(cm,e,"contextmenu"))cm.display.input.onContextMenu(e)}function themeChanged(cm){cm.display.wrapper.className=
cm.display.wrapper.className.replace(/\s*cm-s-\S+/g,"")+cm.options.theme.replace(/(^|\s)\s*/g," cm-s-");clearCaches(cm)}function guttersChanged(cm){updateGutters(cm);regChange(cm);alignHorizontally(cm)}function dragDropChanged(cm,value,old){!value!=!(old&&old!=Init)&&(old=cm.display.dragFunctions,value=value?on:off,value(cm.display.scroller,"dragstart",old.start),value(cm.display.scroller,"dragenter",old.enter),value(cm.display.scroller,"dragover",old.over),value(cm.display.scroller,"dragleave",old.leave),
value(cm.display.scroller,"drop",old.drop))}function wrappingChanged(cm){cm.options.lineWrapping?(addClass(cm.display.wrapper,"CodeMirror-wrap"),cm.display.sizer.style.minWidth="",cm.display.sizerWidth=null):(rmClass(cm.display.wrapper,"CodeMirror-wrap"),findMaxLine(cm));estimateLineHeights(cm);regChange(cm);clearCaches(cm);setTimeout(function(){return updateScrollbars(cm)},100)}function CodeMirror$1(place,options){var this$1=this;if(!(this instanceof CodeMirror$1))return new CodeMirror$1(place,options);
this.options=options=options?copyObj(options):{};copyObj(defaults,options,!1);setGuttersForLineNumbers(options);var doc=options.value;"string"==typeof doc&&(doc=new Doc(doc,options.mode,null,options.lineSeparator,options.direction));this.doc=doc;var input=new CodeMirror$1.inputStyles[options.inputStyle](this);place=this.display=new Display(place,doc,input);place.wrapper.CodeMirror=this;updateGutters(this);themeChanged(this);options.lineWrapping&&(this.display.wrapper.className+=" CodeMirror-wrap");
...
!0;attachDoc(this,doc);options.autofocus&&!mobile||this.hasFocus()?setTimeout(bind(onFocus,this),20):onBlur(this);for(var opt in optionHandlers)if(optionHandlers.hasOwnProperty(opt))optionHandlers[opt](this$1,options[opt],Init);maybeUpdateLineNumberWidth(this);options.finishInit&&options.finishInit(this);for(doc=0;doc<initHooks.length;++doc)initHooks[doc](this$1);endOperation(this);webkit&&options.lineWrapping&&"optimizelegibility"==getComputedStyle(place.lineDiv).textRendering&&(place.lineDiv.style.textRendering=
"auto")}function registerEventHandlers(cm){function finishTouch(){d.activeTouch&&(touchFinished=setTimeout(function(){return d.activeTouch=null},1E3),prevTouch=d.activeTouch,prevTouch.end=+new Date)}function farAway(touch,other){if(null==other.left)return!0;var dx=other.left-touch.left;touch=other.top-touch.top;return 400<dx*dx+touch*touch}var d=cm.display;on(d.scroller,"mousedown",operation(cm,onMouseDown));ie&&11>ie_version?on(d.scroller,"dblclick",operation(cm,function(e){if(!signalDOMEvent(cm,
e)){var pos=posFromMouse(cm,e);!pos||gutterEvent(cm,e,"gutterClick",!0)||eventInWidget(cm.display,e)||(e_preventDefault(e),e=cm.findWordAt(pos),extendSelection(cm.doc,e.anchor,e.head))}})):on(d.scroller,"dblclick",function(e){return signalDOMEvent(cm,e)||e_preventDefault(e)});captureRightClick||on(d.scroller,"contextmenu",function(e){return onContextMenu(cm,e)});var touchFinished,prevTouch={end:0};on(d.scroller,"touchstart",function(e){var JSCompiler_temp;if(JSCompiler_temp=!signalDOMEvent(cm,e))1!=
e.touches.length?JSCompiler_temp=!1:(JSCompiler_temp=e.touches[0],JSCompiler_temp=1>=JSCompiler_temp.radiusX&&1>=JSCompiler_temp.radiusY),JSCompiler_temp=!JSCompiler_temp;JSCompiler_temp&&!gutterEvent(cm,e,"gutterClick",!0)&&(d.input.ensurePolled(),clearTimeout(touchFinished),JSCompiler_temp=+new Date,d.activeTouch={start:JSCompiler_temp,moved:!1,prev:300>=JSCompiler_temp-prevTouch.end?prevTouch:null},1==e.touches.length&&(d.activeTouch.left=e.touches[0].pageX,d.activeTouch.top=e.touches[0].pageY))});
on(d.scroller,"touchmove",function(){d.activeTouch&&(d.activeTouch.moved=!0)});on(d.scroller,"touchend",function(e){var touch=d.activeTouch;if(touch&&!eventInWidget(d,e)&&null!=touch.left&&!touch.moved&&300>new Date-touch.start){var pos=cm.coordsChar(d.activeTouch,"page");touch=!touch.prev||farAway(touch,touch.prev)?new Range(pos,pos):!touch.prev.prev||farAway(touch,touch.prev.prev)?cm.findWordAt(pos):new Range(Pos(pos.line,0),clipPos(cm.doc,Pos(pos.line+1,0)));cm.setSelection(touch.anchor,touch.head);
cm.focus();e_preventDefault(e)}finishTouch()});on(d.scroller,"touchcancel",finishTouch);on(d.scroller,"scroll",function(){d.scroller.clientHeight&&(updateScrollTop(cm,d.scroller.scrollTop),setScrollLeft(cm,d.scroller.scrollLeft,!0),signal(cm,"scroll",cm))});on(d.scroller,"mousewheel",function(e){return onScrollWheel(cm,e)});on(d.scroller,"DOMMouseScroll",function(e){return onScrollWheel(cm,e)});on(d.wrapper,"scroll",function(){return d.wrapper.scrollTop=d.wrapper.scrollLeft=0});d.dragFunctions={enter:function(e){signalDOMEvent(cm,
e)||e_stop(e)},over:function(e){if(!signalDOMEvent(cm,e)){var pos=posFromMouse(cm,e);if(pos){var frag=document.createDocumentFragment();drawSelectionCursor(cm,pos,frag);cm.display.dragCursor||(cm.display.dragCursor=elt$jscomp$0("div",null,"CodeMirror-cursors CodeMirror-dragcursors"),cm.display.lineSpace.insertBefore(cm.display.dragCursor,cm.display.cursorDiv));removeChildrenAndAdd(cm.display.dragCursor,frag)}e_stop(e)}},start:function(e){if(ie&&(!cm.state.draggingText||100>+new Date-lastDrop))e_stop(e);
else if(!signalDOMEvent(cm,e)&&!eventInWidget(cm.display,e)&&(e.dataTransfer.setData("Text",cm.getSelection()),e.dataTransfer.effectAllowed="copyMove",e.dataTransfer.setDragImage&&!safari)){var img=elt$jscomp$0("img",null,null,"position: fixed; left: 0; top: 0;");img.src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw\x3d\x3d";presto&&(img.width=img.height=1,cm.display.wrapper.appendChild(img),img._top=img.offsetTop);e.dataTransfer.setDragImage(img,0,0);presto&&img.parentNode.removeChild(img)}},
drop:operation(cm,onDrop),leave:function(e){signalDOMEvent(cm,e)||clearDragCursor(cm)}};var inp=d.input.getField();on(inp,"keyup",function(e){return onKeyUp.call(cm,e)});on(inp,"keydown",operation(cm,onKeyDown));on(inp,"keypress",operation(cm,onKeyPress));on(inp,"focus",function(e){return onFocus(cm,e)});on(inp,"blur",function(e){return onBlur(cm,e)})}function indentLine(cm,n,how,aggressive){var doc=cm.doc,state;null==how&&(how="add");"smart"==how&&(doc.mode.indent?state=getContextBefore(cm,n).state:
how="prev");var tabSize=cm.options.tabSize,line=getLine(doc,n),curSpace=countColumn(line.text,null,tabSize);line.stateAfter&&(line.stateAfter=null);var curSpaceString=line.text.match(/^\s*/)[0];if(!aggressive&&!/\S/.test(line.text)){var indentation=0;how="not"}else if("smart"==how&&(indentation=doc.mode.indent(state,line.text.slice(curSpaceString.length),line.text),indentation==Pass||150<indentation)){if(!aggressive)return;how="prev"}"prev"==how?indentation=n>doc.first?countColumn(getLine(doc,n-1).text,
null,tabSize):0:"add"==how?indentation=curSpace+cm.options.indentUnit:"subtract"==how?indentation=curSpace-cm.options.indentUnit:"number"==typeof how&&(indentation=curSpace+how);indentation=Math.max(0,indentation);how="";aggressive=0;if(cm.options.indentWithTabs)for(cm=Math.floor(indentation/tabSize);cm;--cm)aggressive+=tabSize,how+="\t";aggressive<indentation&&(how+=spaceStr(indentation-aggressive));if(how!=curSpaceString)return replaceRange(doc,how,Pos(n,0),Pos(n,curSpaceString.length),"+input"),
line.stateAfter=null,!0;for(tabSize=0;tabSize<doc.sel.ranges.length;tabSize++)if(line=doc.sel.ranges[tabSize],line.head.line==n&&line.head.ch<curSpaceString.length){n=Pos(n,curSpaceString.length);replaceOneSelection(doc,tabSize,new Range(n,n));break}}function applyTextInput(cm,inserted,deleted,sel,origin){var doc=cm.doc;cm.display.shift=!1;sel||(sel=doc.sel);var paste=cm.state.pasteIncoming||"paste"==origin,textLines=splitLinesAuto(inserted),multiPaste=null;if(paste&&1<sel.ranges.length)if(lastCopied&&
lastCopied.text.join("\n")==inserted){if(0==sel.ranges.length%lastCopied.text.length){multiPaste=[];for(var i=0;i<lastCopied.text.length;i++)multiPaste.push(doc.splitLines(lastCopied.text[i]))}}else textLines.length==sel.ranges.length&&cm.options.pasteLinesPerSelection&&(multiPaste=map(textLines,function(l){return[l]}));for(i=sel.ranges.length-1;0<=i;i--){var updateInput=sel.ranges[i];var from=updateInput.from(),to=updateInput.to();updateInput.empty()&&(deleted&&0<deleted?from=Pos(from.line,from.ch-
deleted):cm.state.overwrite&&!paste?to=Pos(to.line,Math.min(getLine(doc,to.line).text.length,to.ch+lst(textLines).length)):lastCopied&&lastCopied.lineWise&&lastCopied.text.join("\n")==inserted&&(from=to=Pos(from.line,0)));updateInput=cm.curOp.updateInput;from={from:from,to:to,text:multiPaste?multiPaste[i%multiPaste.length]:textLines,origin:origin||(paste?"paste":cm.state.cutIncoming?"cut":"+input")};makeChange(cm.doc,from);signalLater(cm,"inputRead",cm,from)}inserted&&!paste&&triggerElectric(cm,inserted);
ensureCursorVisible(cm);cm.curOp.updateInput=updateInput;cm.curOp.typing=!0;cm.state.pasteIncoming=cm.state.cutIncoming=!1}function handlePaste(e,cm){var pasted=e.clipboardData&&e.clipboardData.getData("Text");if(pasted)return e.preventDefault(),cm.isReadOnly()||cm.options.disableInput||runInOp(cm,function(){return applyTextInput(cm,pasted,0,null,"paste")}),!0}function triggerElectric(cm,inserted){if(cm.options.electricChars&&cm.options.smartIndent)for(var sel=cm.doc.sel,i=sel.ranges.length-1;0<=
i;i--){var range$$1=sel.ranges[i];if(!(100<range$$1.head.ch||i&&sel.ranges[i-1].head.line==range$$1.head.line)){var mode=cm.getModeAt(range$$1.head),indented=!1;if(mode.electricChars)for(var j=0;j<mode.electricChars.length;j++){if(-1<inserted.indexOf(mode.electricChars.charAt(j))){indented=indentLine(cm,range$$1.head.line,"smart");break}}else mode.electricInput&&mode.electricInput.test(getLine(cm.doc,range$$1.head.line).text.slice(0,range$$1.head.ch))&&(indented=indentLine(cm,range$$1.head.line,"smart"));
indented&&signalLater(cm,"electricInput",cm,range$$1.head.line)}}}function copyableRanges(cm){for(var text=[],ranges=[],i=0;i<cm.doc.sel.ranges.length;i++){var line=cm.doc.sel.ranges[i].head.line;line={anchor:Pos(line,0),head:Pos(line+1,0)};ranges.push(line);text.push(cm.getRange(line.anchor,line.head))}return{text:text,ranges:ranges}}function disableBrowserMagic(field,spellcheck){field.setAttribute("autocorrect","off");field.setAttribute("autocapitalize","off");field.setAttribute("spellcheck",!!spellcheck)}
function hiddenTextarea(){var te=elt$jscomp$0("textarea",null,null,"position: absolute; bottom: -1em; padding: 0; width: 1px; height: 1em; outline: none"),div=elt$jscomp$0("div",[te],null,"overflow: hidden; position: relative; width: 3px; height: 0px;");webkit?te.style.width="1000px":te.setAttribute("wrap","off");ios&&(te.style.border="1px solid black");disableBrowserMagic(te);return div}function findPosH(doc,pos,dir,unit,visually){function moveOnce(boundToLine){var next=visually?moveVisually(doc.cm,
lineObj,pos,dir):moveLogically(lineObj,pos,dir);if(null==next){if(boundToLine=!boundToLine)boundToLine=pos.line+dir,boundToLine<doc.first||boundToLine>=doc.first+doc.size?boundToLine=!1:(pos=new Pos(boundToLine,pos.ch,pos.sticky),boundToLine=lineObj=getLine(doc,boundToLine));if(boundToLine)pos=endOfLine(visually,doc.cm,lineObj,pos.line,dir);else return!1}else pos=next;return!0}var oldPos=pos,origDir=dir,lineObj=getLine(doc,pos.line);if("char"==unit)moveOnce();else if("column"==unit)moveOnce(!0);else if("word"==
unit||"group"==unit){var sawType=null;unit="group"==unit;for(var helper=doc.cm&&doc.cm.getHelper(pos,"wordChars"),first=!0;!(0>dir)||moveOnce(!first);first=!1){var cur=lineObj.text.charAt(pos.ch)||"\n";cur=isWordChar(cur,helper)?"w":unit&&"\n"==cur?"n":!unit||/\s/.test(cur)?null:"p";!unit||first||cur||(cur="s");if(sawType&&sawType!=cur){0>dir&&(dir=1,moveOnce(),pos.sticky="after");break}cur&&(sawType=cur);if(0<dir&&!moveOnce(!first))break}}origDir=skipAtomic(doc,pos,oldPos,origDir,!0);equalCursorPos(oldPos,
origDir)&&(origDir.hitSide=!0);return origDir}function findPosV(cm,pos,dir,unit){var doc=cm.doc,x=pos.left;if("page"==unit){var y=Math.max(Math.min(cm.display.wrapper.clientHeight,window.innerHeight||document.documentElement.clientHeight)-.5*textHeight(cm.display),3);y=(0<dir?pos.bottom:pos.top)+dir*y}else"line"==unit&&(y=0<dir?pos.bottom+3:pos.top-3);for(;;){pos=coordsChar(cm,x,y);if(!pos.outside)break;if(0>dir?0>=y:y>=doc.height){pos.hitSide=!0;break}y+=5*dir}return pos}function posToDOM(cm,pos){var view=
findViewForLine(cm,pos.line);if(!view||view.hidden)return null;var line=getLine(cm.doc,pos.line);view=mapFromLineView(view,line,pos.line);cm=getOrder(line,cm.doc.direction);line="left";cm&&(line=getBidiPartAt(cm,pos.ch)%2?"right":"left");pos=nodeAndOffsetInLineMap(view.map,pos.ch,line);pos.offset="right"==pos.collapse?pos.end:pos.start;return pos}function isInGutter(node){for(;node;node=node.parentNode)if(/CodeMirror-gutter-wrapper/.test(node.className))return!0;return!1}function badPos(pos,bad){bad&&
(pos.bad=!0);return pos}function domTextBetween(cm,from,to,fromLine,toLine){function recognizeMarker(id){return function(marker){return marker.id==id}}function addText(str){str&&(closing&&(text+=lineSep,closing=!1),text+=str)}function walk(node){if(1==node.nodeType){var cmText=node.getAttribute("cm-text");if(null!=cmText)addText(cmText||node.textContent.replace(/\u200b/g,""));else{cmText=node.getAttribute("cm-marker");var range$$1;if(cmText)node=cm.findMarks(Pos(fromLine,0),Pos(toLine+1,0),recognizeMarker(+cmText)),
node.length&&(range$$1=node[0].find(0))&&addText(getBetween(cm.doc,range$$1.from,range$$1.to).join(lineSep));else if("false"!=node.getAttribute("contenteditable")){(range$$1=/^(pre|div|p)$/i.test(node.nodeName))&&closing&&(text+=lineSep,closing=!1);for(cmText=0;cmText<node.childNodes.length;cmText++)walk(node.childNodes[cmText]);range$$1&&(closing=!0)}}}else 3==node.nodeType&&addText(node.nodeValue)}for(var text="",closing=!1,lineSep=cm.doc.lineSeparator();;){walk(from);if(from==to)break;from=from.nextSibling}return text}
function domToPos(cm,node,offset){if(node==cm.display.lineDiv){var lineNode=cm.display.lineDiv.childNodes[offset];if(!lineNode)return badPos(cm.clipPos(Pos(cm.display.viewTo-1)),!0);node=null;offset=0}else for(lineNode=node;;lineNode=lineNode.parentNode){if(!lineNode||lineNode==cm.display.lineDiv)return null;if(lineNode.parentNode&&lineNode.parentNode==cm.display.lineDiv)break}for(var i=0;i<cm.display.view.length;i++){var lineView=cm.display.view[i];if(lineView.node==lineNode)return locateNodeInLineView(lineView,
node,offset)}}function locateNodeInLineView(lineView,node,offset$jscomp$0){function find(textNode,topNode,offset){for(var i=-1;i<(maps?maps.length:0);i++)for(var map$$1=0>i?measure.map:maps[i],j=0;j<map$$1.length;j+=3){var curNode=map$$1[j+2];if(curNode==textNode||curNode==topNode){topNode=lineNo(0>i?lineView.line:lineView.rest[i]);i=map$$1[j]+offset;if(0>offset||curNode!=textNode)i=map$$1[j+(offset?1:0)];return Pos(topNode,i)}}}var wrapper=lineView.text.firstChild,bad=!1;if(!node||!contains(wrapper,
node))return badPos(Pos(lineNo(lineView.line),0),!0);if(node==wrapper&&(bad=!0,node=wrapper.childNodes[offset$jscomp$0],offset$jscomp$0=0,!node))return offset$jscomp$0=lineView.rest?lst(lineView.rest):lineView.line,badPos(Pos(lineNo(offset$jscomp$0),offset$jscomp$0.text.length),bad);var textNode=3==node.nodeType?node:null,topNode=node;textNode||1!=node.childNodes.length||3!=node.firstChild.nodeType||(textNode=node.firstChild,offset$jscomp$0&&(offset$jscomp$0=textNode.nodeValue.length));for(;topNode.parentNode!=
wrapper;)topNode=topNode.parentNode;var measure=lineView.measure,maps=measure.maps;if(node=find(textNode,topNode,offset$jscomp$0))return badPos(node,bad);wrapper=topNode.nextSibling;for(textNode=textNode?textNode.nodeValue.length-offset$jscomp$0:0;wrapper;wrapper=wrapper.nextSibling){if(node=find(wrapper,wrapper.firstChild,0))return badPos(Pos(node.line,node.ch-textNode),bad);textNode+=wrapper.textContent.length}for(topNode=topNode.previousSibling;topNode;topNode=topNode.previousSibling){if(node=
...
qtwebkit=webkit&&/Qt\/\d+\.\d+/.test(userAgent),chrome=!edge&&/Chrome\//.test(userAgent),presto=/Opera\//.test(userAgent),safari=/Apple Computer/.test(navigator.vendor),mac_geMountainLion=/Mac OS X 1\d\D([8-9]|\d\d)\D/.test(userAgent),phantom=/PhantomJS/.test(userAgent),ios=!edge&&/AppleWebKit/.test(userAgent)&&/Mobile\/\w+/.test(userAgent),android=/Android/.test(userAgent),mobile=ios||android||/webOS|BlackBerry|Opera Mini|Opera Mobi|IEMobile/i.test(userAgent),mac=ios||/Mac/.test(platform),chromeOS=
/\bCrOS\b/.test(userAgent),windows=/win/i.test(platform),presto_version=presto&&userAgent.match(/Version\/(\d*\.\d*)/);presto_version&&(presto_version=Number(presto_version[1]));presto_version&&15<=presto_version&&(presto=!1,webkit=!0);var flipCtrlCmd=mac&&(qtwebkit||presto&&(null==presto_version||12.11>presto_version)),captureRightClick=gecko||ie&&9<=ie_version,rmClass=function(node,cls){var current=node.className;if(cls=classTest(cls).exec(current)){var after=current.slice(cls.index+cls[0].length);
...
null);this.fastPoll()};TextareaInput.prototype.onContextMenu=function(e){function prepareSelectAllHack(){if(null!=te.selectionStart){var selected=cm.somethingSelected(),extval="​"+(selected?te.value:"");te.value="⇚";te.value=extval;input.prevInput=selected?"":"​";te.selectionStart=1;te.selectionEnd=extval.length;display.selForContextMenu=cm.doc.sel}}function rehide(){input.contextMenuPending=!1;input.wrapper.style.cssText=oldWrapperCSS;te.style.cssText=oldCSS;ie&&9>ie_version&&display.scrollbars.setScrollTop(display.scroller.scrollTop=
scrollPos);if(null!=te.selectionStart){(!ie||ie&&9>ie_version)&&prepareSelectAllHack();var i=0,poll=function(){display.selForContextMenu==cm.doc.sel&&0==te.selectionStart&&0<te.selectionEnd&&"​"==input.prevInput?operation(cm,selectAll)(cm):10>i++?display.detectingSelectAll=setTimeout(poll,500):(display.selForContextMenu=null,display.input.reset())};display.detectingSelectAll=setTimeout(poll,200)}}var input=this,cm=input.cm,display=cm.display,te=input.textarea,pos=posFromMouse(cm,e),scrollPos=display.scroller.scrollTop;
if(pos&&!presto){cm.options.resetSelectionOnContextMenu&&-1==cm.doc.sel.contains(pos)&&operation(cm,setSelection)(cm.doc,simpleSelection(pos),sel_dontScroll);var oldCSS=te.style.cssText,oldWrapperCSS=input.wrapper.style.cssText;input.wrapper.style.cssText="position: absolute";pos=input.wrapper.getBoundingClientRect();te.style.cssText="position: absolute; width: 30px; height: 30px;\n      top: "+(e.clientY-pos.top-5)+"px; left: "+(e.clientX-pos.left-5)+"px;\n      z-index: 1000; background: "+(ie?
...
var mouseup=function(){off(window,"mouseup",mouseup);setTimeout(rehide,20)};on(window,"mouseup",mouseup)}else setTimeout(rehide,50)}};TextareaInput.prototype.readOnlyChanged=function(val){val||this.reset();this.textarea.disabled="nocursor"==val};TextareaInput.prototype.setUneditable=function(){};TextareaInput.prototype.needsContentAttribute=!1;(function(CodeMirror){function option(name,deflt,handle,notOnInit){CodeMirror.defaults[name]=deflt;handle&&(optionHandlers[name]=notOnInit?function(cm,val,
old){old!=Init&&handle(cm,val,old)}:handle)}var optionHandlers=CodeMirror.optionHandlers;CodeMirror.defineOption=option;CodeMirror.Init=Init;option("value","",function(cm,val){return cm.setValue(val)},!0);option("mode",null,function(cm,val){cm.doc.modeOption=val;loadMode(cm)},!0);option("indentUnit",2,loadMode,!0);option("indentWithTabs",!1);option("smartIndent",!0);option("tabSize",4,function(cm){resetModeState(cm);clearCaches(cm);regChange(cm)},!0);option("lineSeparator",null,function(cm,val){if(cm.doc.lineSep=
val){var newBreaks=[],lineNo=cm.doc.first;cm.doc.iter(function(line){for(var pos=0;;){var found=line.text.indexOf(val,pos);if(-1==found)break;pos=found+val.length;newBreaks.push(Pos(lineNo,found))}lineNo++});for(var i=newBreaks.length-1;0<=i;i--)replaceRange(cm.doc,val,newBreaks[i],Pos(newBreaks[i].line,newBreaks[i].ch+val.length))}});option("specialChars",/[\u0000-\u001f\u007f-\u009f\u00ad\u061c\u200b-\u200f\u2028\u2029\ufeff]/g,function(cm,val,old){cm.state.specialChars=new RegExp(val.source+(val.test("\t")?
"":"|\t"),"g");old!=Init&&cm.refresh()});option("specialCharPlaceholder",defaultSpecialCharPlaceholder,function(cm){return cm.refresh()},!0);option("electricChars",!0);option("inputStyle",mobile?"contenteditable":"textarea",function(){throw Error("inputStyle can not (yet) be changed in a running editor");},!0);option("spellcheck",!1,function(cm,val){return cm.getInputField().spellcheck=val},!0);option("rtlMoveVisually",!windows);option("wholeLineUpdateBefore",!0);option("theme","default",function(cm){themeChanged(cm);
guttersChanged(cm)},!0);option("keyMap","default",function(cm,val,old){val=getKeyMap(val);(old=old!=Init&&getKeyMap(old))&&old.detach&&old.detach(cm,val);val.attach&&val.attach(cm,old||null)});option("extraKeys",null);option("configureMouse",null);option("lineWrapping",!1,wrappingChanged,!0);option("gutters",[],function(cm){setGuttersForLineNumbers(cm.options);guttersChanged(cm)},!0);option("fixedGutter",!0,function(cm,val){cm.display.gutters.style.left=val?compensateForHScroll(cm.display)+"px":"0";
cm.refresh()},!0);option("coverGutterNextToScrollbar",!1,function(cm){return updateScrollbars(cm)},!0);option("scrollbarStyle","native",function(cm){initScrollbars(cm);updateScrollbars(cm);cm.display.scrollbars.setScrollTop(cm.doc.scrollTop);cm.display.scrollbars.setScrollLeft(cm.doc.scrollLeft)},!0);option("lineNumbers",!1,function(cm){setGuttersForLineNumbers(cm.options);guttersChanged(cm)},!0);option("firstLineNumber",1,guttersChanged,!0);option("lineNumberFormatter",function(integer){return integer},
guttersChanged,!0);option("showCursorWhenSelecting",!1,updateSelection,!0);option("resetSelectionOnContextMenu",!0);option("lineWiseCopyCut",!0);option("pasteLinesPerSelection",!0);option("readOnly",!1,function(cm,val){"nocursor"==val&&(onBlur(cm),cm.display.input.blur());cm.display.input.readOnlyChanged(val)});option("disableInput",!1,function(cm,val){val||cm.display.input.reset()},!0);option("dragDrop",!0,dragDropChanged);option("allowDropFileTypes",null);option("cursorBlinkRate",530);option("cursorScrollMargin",
0);option("cursorHeight",1,updateSelection,!0);option("singleCursorHeightPerLine",!0,updateSelection,!0);option("workTime",100);option("workDelay",100);option("flattenSpans",!0,resetModeState,!0);option("addModeClass",!1,resetModeState,!0);option("pollInterval",100);option("undoDepth",200,function(cm,val){return cm.doc.history.undoDepth=val});option("historyEventDelay",1250);option("viewportMargin",10,function(cm){return cm.refresh()},!0);option("maxHighlightLength",1E4,resetModeState,!0);option("moveInputWithCursor",
!0,function(cm,val){val||cm.display.input.resetPosition()});option("tabindex",null,function(cm,val){return cm.display.input.getField().tabIndex=val||""});option("autofocus",null);option("direction","ltr",function(cm,val){return cm.doc.setDirection(val)},!0)})(CodeMirror$1);(function(CodeMirror){var optionHandlers=CodeMirror.optionHandlers,helpers=CodeMirror.helpers={};CodeMirror.prototype={constructor:CodeMirror,focus:function(){window.focus();this.display.input.focus()},setOption:function(option,
value){var options=this.options,old=options[option];if(options[option]!=value||"mode"==option)options[option]=value,optionHandlers.hasOwnProperty(option)&&operation(this,optionHandlers[option])(this,value,old),signal(this,"optionChange",this,option)},getOption:function(option){return this.options[option]},getDoc:function(){return this.doc},addKeyMap:function(map$$1,bottom){this.state.keyMaps[bottom?"push":"unshift"](getKeyMap(map$$1))},removeKeyMap:function(map$$1){for(var maps=this.state.keyMaps,
i=0;i<maps.length;++i)if(maps[i]==map$$1||maps[i].name==map$$1)return maps.splice(i,1),!0},addOverlay:methodOp(function(spec,options){var mode=spec.token?spec:CodeMirror.getMode(this.options,spec);if(mode.startState)throw Error("Overlays may not be stateful.");insertSorted(this.state.overlays,{mode:mode,modeSpec:spec,opaque:options&&options.opaque,priority:options&&options.priority||0},function(overlay){return overlay.priority});this.state.modeGen++;regChange(this)}),removeOverlay:methodOp(function(spec){for(var overlays=
...
e_preventDefault;CodeMirror.e_stopPropagation=e_stopPropagation;CodeMirror.e_stop=e_stop;CodeMirror.addClass=addClass;CodeMirror.contains=contains;CodeMirror.rmClass=rmClass;CodeMirror.keyNames=keyNames})(CodeMirror$1);CodeMirror$1.version="5.31.0";return CodeMirror$1})}
